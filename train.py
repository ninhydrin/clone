...
class train:
    ...
    def train(self, lr, lr_decay, dropout, epoch, b_size, grad_clip):
            batchsize   = args.batchsize
            # arguments
            parser = argparse.ArgumentParser()
            parser.add_argument('--gpu',                        type=int,   default=-1)
...
for i in range(jump * epochs):
    ...
    if (i + 1) % bprop_len == 0:  # Run truncated BPTT
        now = time.time()
        sys.stderr.write('\r{}/{}, train_loss = {}, time = {:.2f}'.format((i+1)/bprop_len,(jump*epochs)/bprop_len, accum_loss.data / bprop_len, now-cur_at))
        sys.stderr.flush()
        #print '{}/{}, train_loss = {}, time = {:.2f}'.format((i+1)/bprop_len,(jump*n_epochs)/bprop_len, accum_loss.data / bprop_len, now-cur_at)
        cur_at = now
